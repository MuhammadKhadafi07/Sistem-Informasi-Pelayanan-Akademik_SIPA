<x-module.pengajuanlegalisir.admin>
    <x-template.button.back-button url="pengajuanlegalisir/admin/legalisirselesai" />
    <div class="card">
        <div class="card-header">
            <form action="{{ url('pengajuanlegalisir/admin/legalisirselesai/clear', $pengajuanlegalisir->id) }}"
                method="post" onsubmit="return confirm('Yakin ingin clear pengajuan ini?')">
                @csrf
                @method('put')
                <button class="btn float-right btn-success btn-sm"><i class="fas fa-user-check"></i>
                    Clear</button>
            </form>
            <form action="{{ url('pengajuanlegalisir/admin/legalisirselesai/proses', $pengajuanlegalisir->id) }}"
                method="post" onsubmit="return confirm('Yakin ingin membatalkan pengajuan ini dari legalisir selesai?')">
                @csrf
                @method('put')
