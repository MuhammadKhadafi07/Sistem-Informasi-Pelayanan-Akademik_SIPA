            ...
            <table class="table table-bordered table-striped table-datatable">
                ...
                <tbody>
                    @php
                        $no = 1;
                    @endphp
                    @foreach ($list_pengumpulanberkas->sortByDesc('created_at')->values() as $pengumpulanberkas)
                        @if ($pengumpulanberkas->status_perpustakaan == 1)
                            <tr>
                                <td>{{ $no++ }}</td>
                                <td>
                                    <div class="btn-group">
                                        <a href="{{ url('koor/perpustakaan/pengumpulanberkas', $pengumpulanberkas->id) }}"
                                            class="btn btn-dark btn-sm"><i class="fas fa-info"></i> Detail</a>
                                        <form
                                            action="{{ url('koor/perpustakaan/pengumpulanberkas/konfirmasi', $pengumpulanberkas->id) }}"
                                            method="post"
