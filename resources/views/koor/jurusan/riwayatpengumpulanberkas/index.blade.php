            ...
            <table class="table table-bordered table-striped table-datatable">
                ...
                <tbody>
                    @php
                        $no = 1;
                    @endphp
                    @foreach ($list_riwayatpengumpulanberkas as $pengumpulanberkas)
                        @if ($pengumpulanberkas->status_jurusan == 2)
                            <tr>
                                <td>{{ $no++ }}</td>
                                <td>
                                    <div class="btn-group">
                                        <a href="{{ url('koor/jurusan/riwayatpengumpulanberkas', $pengumpulanberkas->id) }}"
                                            class="btn btn-dark btn-sm"><i class="fas fa-info"></i> Detail</a>
                                        <form
                                            action="{{ url('koor/jurusan/riwayatpengumpulanberkas/cancel', $pengumpulanberkas->id) }}"
                                            method="post"
